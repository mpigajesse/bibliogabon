      ...
      <div className="container-editorial py-16 grid gap-12 lg:grid-cols-[1.4fr_1fr_1fr_1.2fr]">
        ...
        <FooterCol title="Domaines">
          {DOMAINES.slice(0, 6).map((d) => (
            <FLink key={d.slug} to="/domaines/$slug" params={{ slug: d.slug }}>{d.nom}</FLink>
          ))}
        </FooterCol>

        <FooterCol title="Contact">
          <li className="flex items-start gap-2"><MapPin className="size-4 mt-0.5 text-gold shrink-0" /><span>Libreville, République Gabonaise 🇬🇦</span></li>
          <li className="flex items-start gap-2"><Mail className="size-4 mt-0.5 text-gold shrink-0" /><a href="mailto:contact@biblio-gabon.ga" className="hover:text-gold">contact@biblio-gabon.ga</a></li>
          <li className="flex items-start gap-2"><Phone className="size-4 mt-0.5 text-gold shrink-0" /><span>+241 66 15 24 01</span></li>
        </FooterCol>
      </div>
      ...
      <div className="border-t border-white/10">
        <div className="container-editorial py-5 flex flex-col md:flex-row items-center justify-between gap-3 text-xs text-white/60">
          ...
          <div className="flex items-center gap-5">
            <Link to="/confidentialite" className="hover:text-gold">Confidentialité</Link>
...
function FooterCol({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      <h4 className="font-display text-sm font-semibold text-white uppercase tracking-wider mb-4">{title}</h4>
      <ul className="space-y-2.5 text-sm text-white/70">{children}</ul>
    </div>
  );
}

function FLink({ to, params, children }: { to: string; params?: Record<string, string>; children: React.ReactNode }) {
  return (
    <li>
